...
function calc(rows){
  ...
  return {
    ...
    exactitudSku: pct(latest(i,'skus_totales_activos')-latest(i,'skus_con_diferencia_pendiente'), latest(i,'skus_totales_activos')),
    exactitudUnidades: +(100 - pct(latest(i,'unidades_con_diferencia_pendiente'), latest(i,'unidades_teoricas_totales'))).toFixed(1),
...
function trendData(rows){
  ...
  return dates.map(fecha=>{
    ...
    const c=calc(by); return { fecha: fecha.slice(5), OTIF:c.otif, OTD:c.otd, Ocupacion:c.ocupacion, Inventario:c.exactitudSku || c.precision };
  });
}
...
function DataEntry({perfil,onSaved}){
  ...
  async function save(){ setMsg(''); const payload={fecha:form.fecha,responsable:form.responsable,observaciones:form.observaciones||''}; (sectorFields[sector]||[]).forEach(f=>payload[f]=Number(form[f])||0); if(!supabase){setMsg('Supabase no configurado.'); return} const table=sectorTable[sector]; const existing=await supabase.from(table).select('id').eq('fecha',payload.fecha).eq('responsable',payload.responsable).order('created_at',{ascending:false}).limit(1).maybeSingle(); if(existing.error){setMsg(existing.error.message); return;} let result; if(existing.data?.id){ result=await supabase.from(table).update(payload).eq('id',existing.data.id); } else { result=await supabase.from(table).insert(payload); } if(result.error) setMsg(result.error.message); else {setMsg(existing.data?.id ? 'Carga actualizada correctamente.' : 'Carga guardada correctamente.'); onSaved();}}
...
function SectorDashboard({perfil, rows}){
  ...
  if(sectors.includes('Inventario')){
    ...
    cards.push(['SKUs controlados', sum(i,'skus_controlados'), Boxes], ['Exactitud SKU', pct(latest(i,'skus_totales_activos')-latest(i,'skus_con_diferencia_pendiente'), latest(i,'skus_totales_activos')) + '%', Shield], ['Exactitud unidades', (+(100 - pct(latest(i,'unidades_con_diferencia_pendiente'), latest(i,'unidades_teoricas_totales'))).toFixed(1)) + '%', Shield], ['Dif. pendientes', latest(i,'diferencias_pendientes_actuales'), AlertTriangle], ['Diferencias detectadas', sum(i,'diferencias_detectadas'), AlertTriangle], ['Diferencias resueltas', sum(i,'diferencias_resueltas'), CheckCircle2]);
  }
